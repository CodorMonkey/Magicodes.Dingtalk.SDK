...
namespace Magicodes.Dingtalk.SDK.SmartWork
{
    ...
    {
        public EmployeeApi(ILogger<EmployeeApi> logger, IServiceProvider serviceProvider) : base(
        ...
        /// <returns></returns>
        public async Task<QueryOnJobResult> QueryOnJob(string statusList = "2,3,5,-1",
        ...
        /// <returns></returns>
        public async Task<GetHrmListInfoResult> GetHrmListInfo(
            string userIdList, string findFilterList) {
            return await Post<GetHrmListInfoResult>(
                       "topapi/smartwork/hrm/employee/list?access_token={ACCESS_TOKEN}", new {
        ...
        public async Task<QueryPreentryResult> QueryPreentry(int offset, int size) {
            return await Post<QueryPreentryResult>(
                       "topapi/smartwork/hrm/employee/querypreentry?access_token={ACCESS_TOKEN}",
                       new {
                           offset = offset,
                           size   = size
                       });
